...
Before claiming the first item, run `doctor` through the capability-resolved
command from SKILL.md. If neither the project wrapper nor a fully configured
standalone todo-db >= 0.3 CLI supports it, report the skipped preflight; do not
invoke an unsupported wrapper action. Resolve any FAIL before starting. If a
selected command that documents the v0.3 exit contract returns 4 mid-batch,
HALT immediately. Treat an automatic token re-mint as already attempted only
when using a wrapper generated by todo-db >= 0.3. Record every item whose
