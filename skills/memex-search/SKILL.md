...
Confirm the current `--source` values with `memex search --help` rather than
assuming this list is complete; it changes between versions.

### Harnesses memex does not index

A harness with local transcripts but no `memex` source is a retrieval blind
spot, not an absence of history. `memex` hits that mention such a harness are
cross-references where another agent was told about it, never its own
transcript. Say which harness is unindexed and read its store directly.

For `agy` (Antigravity CLI), the native transcripts live outside the index:

```bash
ls ~/.gemini/antigravity-cli/brain/<uuid>/
cat ~/.gemini/antigravity-cli/brain/<uuid>/.system_generated/logs/transcript.jsonl
